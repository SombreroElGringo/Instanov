...
export interface Route {
  /**
   * Which pattern the route should match
   * 
   * @type {string}
   */
  path: string | RegExp,
  ...
  /**
   * Which action of the controller we should invoke
   * 
   * @type {string}
   */
  action: string,
...
export abstract class BaseController {
  /**
   * The definition of routes for this controller
   * 
   * @static
   * @type {Route[]}
  ...
  /**
   * Prefix all the path of our road by this path
   * 
   * @static
   * @type {string}
   * @memberof BaseController
   ...
   * 
   * An instance of a controller handles a single request, there for it needs the req and res as
   * parameters
   * 
   * @param {Request} req 
   * @param {Response} res 
  ...
  static connect(router: Router): void {
    for (const idx in this.routes) {
      ...
      // The path is prefixed from basePath only if the root option is not true on the Route
      const path = (!route.root && this.basePath || '') + route.path

      // We whitelist in case the possible verbs
      if (['all', 'post', 'get', 'patch', 'put', 'delete'].indexOf(verb) === -1) {
        throw new Error('Incorrect verb for: ' + verb)
      }

      // We also whitelist the actions, so we check that their type is a function
      // So here we check on the prototype if the actions are instance methods
